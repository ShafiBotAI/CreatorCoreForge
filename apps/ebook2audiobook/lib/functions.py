...
def check_programs(prog_name, command, options):
    try:
        subprocess.run(
            [command, options], stdout=subprocess.PIPE, stderr=subprocess.PIPE, check=True, text=True, encoding="utf-8"
        )
        return True, None
...
def normalize_text(text, lang, lang_iso1, tts_engine):
    ...
    if lang in abbreviations_mapping:
        ...
        pattern = re.compile(
            r"\b(" + "|".join(re.escape(k).replace("\\.", "") for k in abbreviations_mapping[lang].keys()) + r")\.?\b",
            re.IGNORECASE,
        )
...
*******************************************************************************
        ...
        try:
            ...
            toc_list = [
                normalize_text(str(item.title), session["language"], session["language_iso1"], session["tts_engine"])
                for item in toc
        ...
        all_docs = [item for item in epubBook.get_items_of_type(ebooklib.ITEM_DOCUMENT) if item.id in spine_ids]
        if not all_docs:
            return [], []
...
def convert_chapters2audio(session):
    try:
        ...
        with tqdm(
            ...
            desc="conversion 0.00%",
            bar_format="{desc}: {n_fmt}/{total_fmt} ",
            unit="step",
            initial=resume_sentence,
        ) as t:
            ...
            for x in range(0, total_chapters):
                ...
                for i, sentence in enumerate(sentences):
                    if session["cancellation_requested"]:
                        ...
                        return False
                    if (
                        sentence_number in missing_sentences
                        or sentence_number > resume_sentence
                        or (sentence_number == 0 and resume_sentence == 0)
                    ):
    ...
    try:
        ...
        try:
            process = subprocess.Popen(
                ffmpeg_cmd, env={}, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, encoding="utf-8", errors="ignore"
            )
            ...
            for index, chapter_file in enumerate(chapter_files):
                ...
                duration_ms = len(
                    AudioSegment.from_file(
                        os.path.join(session["chapters_dir"], chapter_file), format=default_audio_proc_format
                    )
                )
            ...
            if session["output_format"] == "wav":
                ...
                if ffmpeg_cover is not None:
                    if (
                        ...
                        or session["output_format"] == "m4a"
                        or session["output_format"] == "m4b"
                        or session["output_format"] == "mp4"
                        or session["output_format"] == "flac"
                    ):
                        ...
                        if ffmpeg_cover.endswith(".png"):
                            ...
                            ffmpeg_cmd += [
                                ...
                                "-disposition:v",
                                "attached_pic",
                            ]  # JPEG cover (no re-encoding needed)
                        ...
                        ffmpeg_cmd += [
                            "-filter_complex",
                            "[2:v:0][0:a:0]concat=n=1:v=1:a=1[outv][rawa];[rawa]loudnorm=I=-16:LRA=11:TP=-1.5,afftdn=nf=-70[outa]",
                            "-map",
                            "[outv]",
                            "-map",
                            "[outa]",
    ...
    try:
        ...
        if len(chapter_files) > 0:
            combined_chapters_file = os.path.join(
                session["process_dir"], get_sanitized(session["metadata"]["title"]) + "." + default_audio_proc_format
            )
...
def replace_roman_numbers(text, lang):
    ...
    roman_numerals_with_period = re.compile(
        r"^(?=[IVXLCDM])((?:M{0,3})(?:CM|CD|D?C{0,3})?(?:XC|XL|L?X{0,3})?(?:IX|IV|V?I{0,3}))\.+", re.IGNORECASE
    )
...
def delete_unused_tmp_dirs(web_dir, days, session):
    dir_array = [tmp_dir, web_dir, os.path.join(models_dir, "__sessions"), os.path.join(voices_dir, "__sessions")]
    current_user_dirs = {
        f"ebook-{session['id']}",
        f"web-{session['id']}",
        f"voice-{session['id']}",
...
def get_compatible_tts_engines(language):
    compatible_engines = [tts for tts in models.keys() if language in language_tts.get(tts, {})]
            ...
            if error is None:
                ...
                if error is None:
                    ...
                    session["process_dir"] = os.path.join(
                        session["session_dir"], f"{hashlib.md5(session['ebook'].encode()).hexdigest()}"
                    )
                    ...
                    if prepare_dirs(args["ebook"], session):
                        ...
                        if convert2epub(session):
                            ...
                            metadata["title"] = (
                                metadata["title"]
                                if metadata["title"]
                                else os.path.splitext(os.path.basename(session["ebook"]))[0].replace("_", " ")
                            )
                            metadata["creator"] = (
                                False
                            ...
                            try:
                                ...
                                print(f"Metadata language mapping failed: {e}")
                                ...
                                if session["chapters"] is not None:
                                    if convert_chapters2audio(session):
                                        ...
                                        if final_file is not None:
                                            ...
                                            shutil.rmtree(
                                                os.path.join(session["voice_dir"], "proc"), ignore_errors=True
                                            )
                                            if is_gui_process:
...
def web_interface(args):
    ...
    with gr.Blocks(theme=theme, delete_cache=(86400, 86400)) as interface:
        ...
        gr_logo_markdown = gr.Markdown(
            elem_id="gr_logo_markdown",
            value=f"""
        ...
        with gr.Tabs():
            ...
            with gr_tab_main:
                with gr.Row():
                    with gr.Column(scale=3):
                        with gr.Group():
                            gr_ebook_file = gr.File(
                                label=src_label_file,
                                elem_id="gr_ebook_file",
                                file_types=ebook_formats,
                                file_count="single",
                                allow_reordering=True,
                                height=140,
                            )
                            gr_ebook_mode = gr.Radio(
                                label="",
                                elem_id="gr_ebook_mode",
                                choices=[("File", "single"), ("Directory", "directory")],
                                value="single",
                                interactive=True,
                            )
                        with gr.Group():
                            gr_language = gr.Dropdown(
                                label="Language",
                                elem_id="gr_language",
                        ...
                        gr_group_voice_file = gr.Group(
                            elem_id="gr_group_voice_file", visible=visible_gr_group_voice_file
                        )
                        with gr_group_voice_file:
                            gr_voice_file = gr.File(
                                label="*Cloning Voice Audio Fiie",
                                elem_id="gr_voice_file",
                                file_types=voice_formats,
                                value=None,
                                height=140,
                            )
                            gr_row_voice_player = gr.Row(elem_id="gr_row_voice_player")
                            with gr_row_voice_player:
                                gr_voice_player = gr.Audio(
                                    elem_id="gr_voice_player",
                                    type="filepath",
                                    interactive=False,
                                ...
                                gr_voice_list = gr.Dropdown(
                                    label="",
                                    elem_id="gr_voice_list",
                                    choices=voice_options,
                                    type="value",
                                    interactive=True,
                                    scale=2,
                                )
                                gr_voice_del_btn = gr.Button(
                                    "🗑",
                                    elem_id="gr_voice_del_btn",
                        ...
                        with gr.Group():
                            gr_device = gr.Radio(
                                ...
                                elem_id="gr_device",
                                choices=[("CPU", "cpu"), ("GPU", "cuda"), ("MPS", "mps")],
                                value=default_device,
                            )
                    with gr.Column(scale=3):
                        with gr.Group():
                            gr_tts_engine_list = gr.Dropdown(
                                label="TTS Engine",
                                elem_id="gr_tts_engine_list",
                                choices=tts_engine_options,
                                type="value",
                                interactive=True,
                            )
                            gr_tts_rating = gr.HTML()
                            gr_fine_tuned_list = gr.Dropdown(
                                label="Fine Tuned Models (Presets)",
                                elem_id="gr_fine_tuned_list",
                                choices=fine_tuned_options,
                                type="value",
                                interactive=True,
                            )
                            gr_group_custom_model = gr.Group(visible=visible_gr_group_custom_model)
                            with gr_group_custom_model:
                                gr_custom_model_file = gr.File(
                                    label=f"Upload Fine Tuned Model",
                                    elem_id="gr_custom_model_file",
                                    value=None,
                                    file_types=[".zip"],
                                    height=140,
                                )
                                with gr.Row():
                                    gr_custom_model_list = gr.Dropdown(
                                        label="",
                                        elem_id="gr_custom_model_list",
                                        choices=custom_model_options,
                                        type="value",
                                        interactive=True,
                                        scale=2,
                                    )
                                    gr_custom_model_del_btn = gr.Button(
                                        "🗑",
                                        elem_id="gr_custom_model_del_btn",
                                        elem_classes=["small-btn"],
                                        variant="secondary",
                                        interactive=True,
                                        visible=False,
                        ...
                        gr_output_format_list = gr.Dropdown(
                            label="Output format",
                            elem_id="gr_output_format_list",
                            choices=output_formats,
                            type="value",
                            value=default_output_format,
                            interactive=True,
                        )
            gr_tab_xtts_params = gr.TabItem(
                "XTTSv2 Fine Tuned Parameters", elem_classes="tab_item", visible=visible_gr_tab_xtts_params
            )
            ...
            gr_tab_bark_params = gr.TabItem(
                "BARK fine Tuned Parameters", elem_classes="tab_item", visible=visible_gr_tab_bark_params
            )
        ...
        with gr_group_audiobook_list:
            gr_audiobook_text = gr.Textbox(
                label="Audiobook", elem_id="gr_audiobook_text", interactive=False, visible=True
            )
            gr_audiobook_player = gr.Audio(
                label="",
                elem_id="gr_audiobook_player",
                type="filepath",
            ...
            with gr.Row():
                gr_audiobook_download_btn = gr.DownloadButton(
                    "↧",
                    elem_id="gr_audiobook_download_btn",
                    elem_classes=["small-btn"],
                    variant="secondary",
                    interactive=True,
                    visible=True,
                ...
                gr_audiobook_list = gr.Dropdown(
                    label="",
                    elem_id="gr_audiobook_list",
                    choices=audiobook_options,
                    type="value",
                    interactive=True,
                    visible=True,
                    scale=2,
                )
                gr_audiobook_del_btn = gr.Button(
                    "🗑",
                    elem_id="gr_audiobook_del_btn",
                    elem_classes=["small-btn"],
                    variant="secondary",
                    interactive=True,
                    visible=True,
        ...
        gr_convert_btn = gr.Button(
            "📚", elem_id="gr_convert_btn", elem_classes="icon-btn", variant="primary", interactive=False
        )
        ...
        def show_rating(tts_engine):
            ...
            def yellow_stars(n):
                return "".join("<span style='color:#FFD700;font-size:12px'>★</span>" for _ in range(n))
        ...
        def restore_interface(id):
            ...
            ### XTTSv2 Params
            session["temperature"] = (
                session["temperature"] if session["temperature"] else default_xtts_settings["temperature"]
            )
            ...
            session["repetition_penalty"] = (
                session["repetition_penalty"]
                if session["repetition_penalty"]
                else default_xtts_settings["repetition_penalty"]
            )
            ...
            session["waveform_temp"] = (
                session["waveform_temp"] if session["waveform_temp"] else default_bark_settings["waveform_temp"]
            )
            return (
                ...
                gr.update(value=float(session["temperature"])),
                gr.update(value=float(session["length_penalty"])),
                gr.update(value=int(session["num_beams"])),
                gr.update(value=float(session["repetition_penalty"])),
                gr.update(value=int(session["top_k"])),
                gr.update(value=float(session["top_p"])),
                gr.update(value=float(session["speed"])),
                gr.update(value=bool(session["enable_text_splitting"])),
                gr.update(value=float(session["text_temp"])),
                gr.update(value=float(session["waveform_temp"])),
        ...
        def refresh_interface(id):
            ...
            return (
                ...
                update_gr_voice_list(id),
                update_gr_audiobook_list(id),
                gr.update(value=session["audiobook"]),
            ...
            try:
                if session is None:
                    ...
                    if hasattr(upload_file, "name") and not hasattr(custom_model_file, "name"):
                        ...
                        isinstance(upload_file, list)
                        and len(upload_file) > 0
                        and upload_file_mode == "directory"
                        and not hasattr(custom_model_file, "name")
                    ):
                        return gr.update(variant="primary", interactive=True)
                    else:
                        return gr.update(variant="primary", interactive=False)
        ...
        def click_gr_voice_del_btn(selected, id):
            try:
                if selected is not None:
                    ...
                    if (
                        voice_name in default_xtts_settings["voices"].keys()
                        or voice_name in default_yourtts_settings["voices"].keys()
                    ):
                        ...
                        try:
                            ...
                            if selected.find(session["voice_dir"]) > -1:
                                ...
                                return gr.update(value="confirm_voice_del"), gr.update(
                                    value=show_modal("confirm", msg), visible=True
                                )
                            else:
        ...
        def click_gr_custom_model_del_btn(selected, id):
            try:
                if selected is not None:
                    ...
                    return gr.update(value="confirm_custom_model_del"), gr.update(
                        value=show_modal("confirm", msg), visible=True
                    )
            except Exception as e:
        ...
        def click_gr_audiobook_del_btn(selected, id):
            try:
                if selected is not None:
                    ...
                    selected_name = os.path.basename(selected)
                    msg = f"Are you sure to delete {selected_name}..."
                    return gr.update(value="confirm_audiobook_del"), gr.update(
                        value=show_modal("confirm", msg), visible=True
                    )
            except Exception as e:
        ...
        def update_gr_voice_list(id):
            try:
                ...
                session = context.get_session(id)
                voice_lang_dir = (
                    session["language"] if session["language"] != "con" else "con-"
                )  # Bypass Windows CON reserved name
                ...
                session["voice"] = (
                    session["voice"]
                    if session["voice"] in [option[1] for option in voice_options]
        ...
        def update_gr_tts_engine_list(id):
            try:
                ...
                session["tts_engine"] = (
                    session["tts_engine"] if session["tts_engine"] in tts_engine_options else tts_engine_options[0]
                )
        ...
        def update_gr_custom_model_list(id):
            try:
                ...
                session["custom_model"] = (
                    session["custom_model"]
                    if session["custom_model"] in [option[1] for option in custom_model_options]
                    else custom_model_options[0][1]
                )
        ...
        def update_gr_fine_tuned_list(id):
            try:
                ...
                fine_tuned_options = [
                    name
                    for name, details in models.get(session["tts_engine"], {}).items()
                    if details.get("lang") == "multi" or details.get("lang") == session["language"]
                ]
                session["fine_tuned"] = (
                    session["fine_tuned"] if session["fine_tuned"] in fine_tuned_options else default_fine_tuned
                )
        ...
        def change_gr_tts_engine_list(engine, id):
            ...
            if session["tts_engine"] == XTTSv2:
                ...
                return (
                    ...
                    update_gr_fine_tuned_list(id),
                    gr.update(
                        label=f"*Upload {session['tts_engine']} Model (Should be a ZIP file with {', '.join(models[session['tts_engine']][default_fine_tuned]['files'])})"
                    ),
                ...
                return (
                    ...
                    gr.update(visible=False),
                    update_gr_fine_tuned_list(id),
                    gr.update(label=f"*Upload Fine Tuned Model not available for {session['tts_engine']}"),
                    gr.update(label=""),
                )
        ...
        def update_gr_audiobook_list(id):
            try:
                ...
                session = context.get_session(id)
                audiobook_options = [
                    (f, os.path.join(session["audiobooks_dir"], str(f))) for f in os.listdir(session["audiobooks_dir"])
                ]
                        ...
                        if session:
                            ...
                            if session["status"] == "converting":
                                if session["progress"] != len(audiobook_options):
                                    session["progress"] = len(audiobook_options)
                                    return (
                                        gr.update(value=json.dumps(session_dict, indent=4)),
        ...
        ).then(fn=change_gr_ebook_file, inputs=[gr_ebook_file, gr_session], outputs=[gr_modal])
        gr_ebook_mode.change(fn=change_gr_ebook_mode, inputs=[gr_ebook_mode, gr_session], outputs=[gr_ebook_file])
        gr_voice_file.upload(fn=change_gr_voice_file, inputs=[gr_voice_file, gr_session], outputs=[gr_voice_file]).then(
            fn=update_gr_voice_list, inputs=[gr_session], outputs=[gr_voice_list]
        )
        gr_voice_list.change(
            fn=change_gr_voice_list, inputs=[gr_voice_list, gr_session], outputs=[gr_voice_player, gr_voice_del_btn]
        )
        gr_voice_del_btn.click(
            fn=click_gr_voice_del_btn, inputs=[gr_voice_list, gr_session], outputs=[gr_confirm_field_hidden, gr_modal]
        )
        gr_device.change(fn=change_gr_device, inputs=[gr_device, gr_session], outputs=None)
        gr_language.change(
        ...
        gr_fine_tuned_list.change(
            fn=change_gr_fine_tuned_list, inputs=[gr_fine_tuned_list, gr_session], outputs=[gr_group_custom_model]
        )
        gr_custom_model_file.upload(
            fn=change_gr_custom_model_file,
            inputs=[gr_custom_model_file, gr_tts_engine_list, gr_session],
            outputs=[gr_custom_model_file],
        ).then(fn=update_gr_custom_model_list, inputs=[gr_session], outputs=[gr_custom_model_list])
        ...
        gr_output_format_list.change(
            fn=change_gr_output_format_list, inputs=[gr_output_format_list, gr_session], outputs=None
        )
        ...
        ).then(fn=None, js="()=>window.redraw_audiobook_player()")
        ...
        gr_xtts_top_k.change(
            fn=lambda val, id: change_param("top_k", val, id), inputs=[gr_xtts_top_k, gr_session], outputs=None
        )
        gr_xtts_top_p.change(
            fn=lambda val, id: change_param("top_p", val, id), inputs=[gr_xtts_top_p, gr_session], outputs=None
        )
        gr_xtts_speed.change(
            fn=lambda val, id: change_param("speed", val, id), inputs=[gr_xtts_speed, gr_session], outputs=None
        )
        ...
        gr_bark_text_temp.change(
            fn=lambda val, id: change_param("text_temp", val, id), inputs=[gr_bark_text_temp, gr_session], outputs=None
        )
        ...
        ).then(fn=clear_event, inputs=[gr_session], outputs=None)
        gr_convert_btn.click(fn=update_convert_btn, inputs=None, outputs=[gr_convert_btn]).then(
            ...
            inputs=[
                ...
                gr_xtts_temperature,
                gr_xtts_length_penalty,
                gr_xtts_num_beams,
                gr_xtts_repetition_penalty,
                gr_xtts_top_k,
                gr_xtts_top_p,
                gr_xtts_speed,
                gr_xtts_enable_text_splitting,
                gr_bark_text_temp,
        ...
        gr_read_data.change(
            fn=change_gr_read_data, inputs=[gr_read_data, gr_state], outputs=[gr_write_data, gr_state, gr_session]
        ).then(
    ...
    try:
        ...
        interface.queue(default_concurrency_limit=interface_concurrency_limit).launch(
